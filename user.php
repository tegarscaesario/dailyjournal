...
if (isset($_POST['simpan'])) {
    ...
    if (isset($_POST['id'])) {
        ...
        if ($nama_foto == '') {
            ...
            if ($_POST['foto_lama'] != '') { unlink("img/" . $_POST['foto_lama']); }
        }

        $stmt = $conn->prepare("UPDATE user SET username =?, password =?, foto = ? WHERE id = ?");
        $stmt->bind_param("sssi", $username_post, $password_post, $foto, $id);
        $simpan = $stmt->execute();
    } else {
        $stmt = $conn->prepare("INSERT INTO user (username, password, foto) VALUES (?,?,?)");
        $stmt->bind_param("sss", $username_post, $password_post, $foto);
        $simpan = $stmt->execute();
    }
